# Spec: Effective Date Eligibility Enforcement — Benefits Determiner

**Pod spec number:** claim-runner/008
**Cpod spec:** claim-runner/cpod/007 (Effective Date Eligibility Enforcement)
**Service scope:** Benefits Determiner only
**Status:** Allocated
...
If `enrollment.effective_date` is null or absent on the member record, the
effective date guard is skipped and determination proceeds to the termination
date check and normal flow.
...
| `enrollment.effective_date` is null or absent on the member record | Effective date guard is skipped; determination proceeds to the termination date check and normal flow |

---

## Constraints

- Benefits Determiner only. No change to Claims Manager, Pricer, or Data Service
  API contracts. (Constitution: Spec Scope)
